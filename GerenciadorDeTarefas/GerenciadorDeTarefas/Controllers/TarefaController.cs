...
namespace GerenciadorDeTarefas.Controllers
{
    ...
    public class TarefaController : BaseController
    {
        ...
        public IActionResult AdicionarTarefa([FromBody] Tarefa tarefa)
        {
            try
            {
                ...
                else
                {
                    ...
                    {
                        erros.Add("Data de previsão nao pode ser menor que hoje");
                    }
                }
            ...
            catch (Exception e)
            {
                ...
                return StatusCode(StatusCodes.Status500InternalServerError, new ErroRespostaDto()
                {
                    ...
                    Erro = "Ocorreu erro ao adicionar tarefa"
                });
            }
        }
        ...
        public IActionResult DeletaTarefa(int idTarefa)
        {
            ...
            {
                var usuario = ReadToken();
                if(usuario == null || idTarefa <= 0) //usuario errado ou nao existe
                {
                    return BadRequest(new ErroRespostaDto()
                    {
                        Status = StatusCodes.Status400BadRequest,
                ...
                if(tarefa == null || tarefa.IdUsuario != usuario.Id) //se a tarefa é da pessoa
                {
                    return BadRequest(new ErroRespostaDto()
                    {
                        Status = StatusCodes.Status400BadRequest,
            ...
            catch (Exception e)
            {
                ...
                {
                    Status = StatusCodes.Status500InternalServerError,
                    Erro = "Ocorreu erro ao deletar tarefa"
                });
            }
        }
        ...
        public IActionResult AtualizarTarefa([FromBody] Tarefa model, int idTarefa)
        {
            try 
            {
                var usuario = ReadToken(); //ler user
                if (usuario == null || idTarefa <= 0) //usuario errado ou nao existe
                {
                    return BadRequest(new ErroRespostaDto()
                    {
                ...
                var tarefa = _tarefaRepository.GetById(idTarefa); //tarefa no banco
            ...
            catch (Exception e)
            {
                ...
                {
                    Status = StatusCodes.Status500InternalServerError,
                    Erro = "Ocorreu erro ao atualizar tarefa"
                });
            }
        }

        [HttpGet] //list
        public IActionResult ListarTarefasUsuario(DateTime? periodoDe, DateTime? periadoAte, StatusTarefaEnum status)
        {
            try
            {
                var usuario = ReadToken();
                ...
                var resultado = _tarefaRepository.BuscarTarefas(usuario.Id, periodoDe, periadoAte, status);
                return Ok(resultado);
            }
            catch (Exception e)
            {
                ...
                {
                    Status = StatusCodes.Status500InternalServerError,
                    Erro = "Ocorreu erro ao listar tarefas"
                });
            }
        }
    }
